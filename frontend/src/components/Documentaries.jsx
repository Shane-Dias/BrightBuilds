  ...
  if (projects.length === 0) {
    return (
      <div className="text-center py-16">
        <FileText className="w-16 h-16 mx-auto mb-4 opacity-50" style={{ color: 'var(--accent-light)' }} />
        <p className="text-lg font-medium" style={{ color: 'var(--text-secondary)' }}>No documentary projects found</p>
      </div>
    );
  }
      ...
      <div className="relative z-10">
        <div className="flex items-center justify-center gap-3 mb-8 sm:mb-12">
          <div className="p-3 rounded-xl" style={{ backgroundColor: 'var(--bg-secondary)' }}>
            <FileText className="w-8 h-8" style={{ color: 'var(--amber-primary)' }} />
          </div>
        ...
        >
          {filteredAndSortedDocumentaries.map(documentary => {
            const rankings = getProjectRankings(documentary._id);
            return (
              <ProjectCard
                key={documentary._id}
                project={documentary}
                onHover={setHoveredDoc}
                isHovered={hoveredDoc === documentary._id}
